...
import { MANGO_MINT, MAX_ACCOUNTS } from 'utils/constants'
...
const CreateAccountForm = ({
  ...
  useEffect(() => {
    ...
    const getBalance = async () => {
      try {
        ...
        const lehPublicKey = new PublicKey(MANGO_MINT);
        const tokenAccount = await getAssociatedTokenAddress(
          lehPublicKey,
          publicKey
        );
        const {value:userBalance} = await connection.getTokenAccountBalance(tokenAccount);
        console.log(userBalance);

        setBalance(parseFloat(userBalance.amount)); // Convert from lamports to SOL
      } catch (error) {
  ...
  if(balance<10000){
    return <div>
          <p className="mt-1 text-center text-th-fail">
          1 LEH required to enter:
          </p>
        <Button
        className="mt-6 w-full"
        disabled={maxSolDeposit <= 0}
        onClick={()=>{
          window.open("https://raydium.io/swap/?outputCurrency=7obg932wg2A1oTZhrPzR4DSdhKbjXzKSfHTBo2Hu5EbP")
        }}
        size="large"
      >
        Buy LEH
